...
Tree.prototype.countLeaves = function () {
  // counter var to return at end
  var leafCount = 0;
  // if not an instance of a tree, return null
  if (!this instanceof Tree) {
    return null;
  }
  // traverse tree
  var traverse = function(node) {
    // if node has no children, increment leaf count
    if (node.children.length === 0) {
      leafCount ++;
    } else {
      // if there is children, loop through them and call traverse on each
      for (var i = 0; i < node.children.length; i ++) {
        traverse(node.children[i]);
      }
    }
  }
  // invoke initial traversal
  traverse(this);
  // return counter
  return leafCount;
};
